...
export default function MenuBarDropdown({ className = "" }: Props) {
  const { user, logout } = useAuthContext();
  return (
    <Popover className={`MenuBarDropdown relative flex ${className}`}>
      {({ open, close }) => (
        <>
          <Popover.Button className="flex h-10 w-10 items-center justify-center self-center rounded-full text-slate-700 hover:bg-slate-100 focus:outline-none dark:text-slate-300 dark:hover:bg-slate-800 sm:h-12 sm:w-12">
